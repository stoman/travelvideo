...
test('middle video gets both previous and next links, plus stop-at', () => {
  ...
  const html = renderTripDisplay(trip, video, 'en');
  assert.match(html, /href="\/en\/trip\/sicily\/taormina">Previous video/);
  assert.match(html, /href="\/en\/trip\/sicily\/etna">Next video/);
  assert.match(html, /href="\/en\/video\/siracusa">Stop at this video/);
...
test('shows the same metadata as /video/:id, per video-info.ts', () => {
  ...
  const video = getVideo('siracusa')!;
  const html = renderTripDisplay(trip, video, 'en');
  assert.match(html, /Country: <a href="\/en\/country\/italy">Italy<\/a>/);
...
test('German: translated nav labels and locale-prefixed hrefs', () => {
  const trip = getTrip('sicily')!;
  const video = getVideo('siracusa')!;
  const html = renderTripDisplay(trip, video, 'de');
  assert.match(html, /href="\/de\/trip\/sicily\/taormina">Vorheriges Video/);
  assert.match(html, /href="\/de\/trip\/sicily\/etna">Nächstes Video/);
  assert.match(html, /href="\/de\/video\/siracusa">Bei diesem Video anhalten/);
});
